...
io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", ({ username, room }) => {
        ...
        socket.emit("emojiCountUpdate", emojiCount);
    });
    ...
    socket.on("sendEmoji", (emoji) => {
        if (!emojiCount[emoji]) emojiCount[emoji] = 0;
        emojiCount[emoji]++;

        io.to(socket.room).emit("emojiCountUpdate", emojiCount);
        io.to(socket.room).emit("message", {
            user: socket.username,
            msg: `sent an emoji: ${emoji}`
        });
    });
